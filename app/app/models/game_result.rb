...
class GameResult < ApplicationRecord
  ...
  def home_win_score_must_be_higher
    errors.add(:result, :invalid) unless home_team_score > visitor_team_score
  end
  ...
  def canceled_scores_must_be_zero
    # The scores must be zero when the game is canceled.
    errors.add(:result, :invalid) unless home_team_score.zero? && visitor_team_score.zero?
  end
end
